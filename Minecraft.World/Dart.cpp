...
const double Dart::DART_BASE_DAMAGE = 2.0f;
...
Dart::Dart(Level *level) : Entity( level )
{
	_init();
...
Dart::Dart(Level *level, shared_ptr<LivingEntity> mob, shared_ptr<LivingEntity> target, float power, float uncertainty) : Entity( level )
{
	_init();
...
Dart::Dart(Level *level, double x, double y, double z) : Entity( level )
{
	_init();
...
Dart::Dart(Level *level, shared_ptr<LivingEntity> mob, float power) : Entity( level )
{
	_init();
...
void Dart::defineSynchedData()
{
	entityData->define(ID_FLAGS, static_cast<byte>(0));
}

void Dart::shoot(double xd, double yd, double zd, float pow, float uncertainty)
{
...
void Dart::lerpTo(double x, double y, double z, float yRot, float xRot, int steps)
{
	setPos(x, y, z);
	setRot(yRot, xRot);
}

void Dart::lerpMotion(double xd, double yd, double zd)
{
	this->xd = xd;
	this->yd = yd;
	this->zd = zd;
...
void Dart::tick() 
{
	...
	if (res != nullptr)
	{
		if (res->entity != nullptr)
		{
			...
			if (owner == nullptr)
			{
				damageSource = DamageSource::dart(dynamic_pointer_cast<Dart>(shared_from_this()), shared_from_this());
			}
			else
			{
				damageSource = DamageSource::dart(dynamic_pointer_cast<Dart>(shared_from_this()), owner);
			}
...
void Dart::addAdditonalSaveData(CompoundTag *tag)
{
...
void Dart::readAdditionalSaveData(CompoundTag *tag)
{
...
void Dart::playerTouch(shared_ptr<Player> player)
{
	...
	{
		if (!player->inventory->add(std::make_shared<ItemInstance>(Item::dartGold, 1)))
		{
			bRemove = false;
		}
	}
...
bool Dart::makeStepSound()
{
	return false;
}

float Dart::getShadowHeightOffs()
{
	return 0;
}

void Dart::setBaseDamage(double baseDamage)
{
	this->baseDamage = baseDamage;
}

double Dart::getBaseDamage()
{
	return baseDamage;
}

void Dart::setKnockback(int knockback)
{
	this->knockback = knockback;
}

bool Dart::isAttackable()
{
	return false;
}

void Dart::setCritDart(bool critDart)
{
