...
const double DartPoison::DART_BASE_DAMAGE = 2.0f;

void DartPoison::_init()
{
...
DartPoison::DartPoison(Level *level) : Entity( level )
{
	_init();
...
DartPoison::DartPoison(Level *level, shared_ptr<LivingEntity> mob, shared_ptr<LivingEntity> target, float power, float uncertainty) : Entity( level )
{
	_init();
...
DartPoison::DartPoison(Level *level, double x, double y, double z) : Entity( level )
{
	_init();
...
DartPoison::DartPoison(Level *level, shared_ptr<LivingEntity> mob, float power) : Entity( level )
{
	_init();
...
void DartPoison::defineSynchedData()
{
	entityData->define(ID_FLAGS, static_cast<byte>(0));
}

void DartPoison::shoot(double xd, double yd, double zd, float pow, float uncertainty)
{
...
void DartPoison::lerpTo(double x, double y, double z, float yRot, float xRot, int steps)
{
	setPos(x, y, z);
	setRot(yRot, xRot);
}

void DartPoison::lerpMotion(double xd, double yd, double zd)
{
	this->xd = xd;
	this->yd = yd;
	this->zd = zd;
...
void DartPoison::tick() 
{
	...
	if (res != nullptr)
	{
		if (res->entity != nullptr)
		{
			...
			if(isCritDartPoison()) dmg += random->nextInt(dmg / 2 + 2);

			DamageSource *damageSource = nullptr;
			if (owner == nullptr)
			{
				damageSource = DamageSource::dartPoison(dynamic_pointer_cast<DartPoison>(shared_from_this()), shared_from_this());
			}
			else
			{
				damageSource = DamageSource::dartPoison(dynamic_pointer_cast<DartPoison>(shared_from_this()), owner);
			}
			...
			if(res->entity->hurt(damageSource, dmg))
			{
				if (res->entity->instanceof(eTYPE_LIVINGENTITY))
				{
					...
					if (!level->isClientSide)
					{
						...
						mob->addEffect(new MobEffectInstance(MobEffect::poison->id, 10 * SharedConstants::TICKS_PER_SECOND, 0));
					}
					if (knockback > 0)
					{
...
void DartPoison::addAdditonalSaveData(CompoundTag *tag)
{
...
void DartPoison::readAdditionalSaveData(CompoundTag *tag)
{
...
void DartPoison::playerTouch(shared_ptr<Player> player)
{
	...
	{
		if (!player->inventory->add(std::make_shared<ItemInstance>(Item::dartPoison, 1)))
		{
			bRemove = false;
		}
	}
...
bool DartPoison::makeStepSound()
{
	return false;
}

float DartPoison::getShadowHeightOffs()
{
	return 0;
}

void DartPoison::setBaseDamage(double baseDamage)
{
	this->baseDamage = baseDamage;
}

double DartPoison::getBaseDamage()
{
	return baseDamage;
}

void DartPoison::setKnockback(int knockback)
{
	this->knockback = knockback;
}

bool DartPoison::isAttackable()
{
	return false;
}

void DartPoison::setCritDartPoison(bool critDartPoison)
{
...
bool DartPoison::isCritDartPoison()
{
	byte flags = entityData->getByte(ID_FLAGS);
