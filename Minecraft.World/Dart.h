...
class Dart : public Entity, public Projectile
{
public:
	eINSTANCEOF GetType() {	return eTYPE_DART; }
	static Entity *create(Level *level) { return new Dart(level); }
...
public:
	...
	Dart(Level *level);
	Dart(Level *level, shared_ptr<LivingEntity> mob, shared_ptr<LivingEntity> target, float power, float uncertainty);
	Dart(Level *level, double x, double y, double z);
	Dart(Level *level, shared_ptr<LivingEntity> mob, float power);
