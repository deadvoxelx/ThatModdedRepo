...
class DartNethanium : public Entity, public Projectile
{
public:
	eINSTANCEOF GetType() {	return eTYPE_DARTNETHANIUM; }
	static Entity *create(Level *level) { return new DartNethanium(level); }
...
public:
	...
	DartNethanium(Level *level);
	DartNethanium(Level *level, shared_ptr<LivingEntity> mob, shared_ptr<LivingEntity> target, float power, float uncertainty);
	DartNethanium(Level *level, double x, double y, double z);
	DartNethanium(Level *level, shared_ptr<LivingEntity> mob, float power);
