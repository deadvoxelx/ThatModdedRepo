...
const double FarlandsLevelSource::SNOW_SCALE = 0.3;
const double FarlandsLevelSource::SNOW_CUTOFF = 0.5;

FarlandsLevelSource::FarlandsLevelSource(Level *level, __int64 seed, bool generateStructures) : generateStructures( generateStructures )
{
...
FarlandsLevelSource::~FarlandsLevelSource()
{
	delete caveFeature;
...
int g_numPrepareHeightCallsF = 0;
LARGE_INTEGER g_totalPrepareHeightsTimeF = {0,0};
LARGE_INTEGER g_averagePrepareHeightsTimeF = {0, 0};
...
int FarlandsLevelSource::getMinDistanceToEdge(int xxx, int zzz, int worldSize, float falloffStart)
{
...
float FarlandsLevelSource::getHeightFalloff(int xxx, int zzz, int* pEMin)
{
	///////////////////////////////////////////////////////////////////
...
void FarlandsLevelSource::prepareHeights(int xOffs, int zOffs, byteArray blocks)
{
...
// We can put the new code back in mid-november 2014 once those PS3/Vita builds are gone (and the PS4 doesn't have world enlarging in these either anyway)
	...
	g_numPrepareHeightCallsF++;
	g_totalPrepareHeightsTimeF.QuadPart += timeInFunc.QuadPart;
	g_averagePrepareHeightsTimeF.QuadPart = g_totalPrepareHeightsTimeF.QuadPart / g_numPrepareHeightCallsF;

	delete [] buffer.data;
	delete [] biomes.data;
}

void FarlandsLevelSource::buildSurfaces(int xOffs, int zOffs, byteArray blocks, BiomeArray biomes)
{
	int waterHeight = level->seaLevel;
...
LevelChunk *FarlandsLevelSource::create(int x, int z)
{
	return getChunk(x,z);
}

LevelChunk *FarlandsLevelSource::getChunk(int xOffs, int zOffs)
{
...
void FarlandsLevelSource::lightChunk(LevelChunk *lc)
{
	lc->recalcHeightmap();
}


doubleArray FarlandsLevelSource::getHeights(doubleArray buffer, int x, int y, int z, int xSize, int ySize, int zSize, BiomeArray& biomes)
{
	if (buffer.data == NULL)
	{
...
bool FarlandsLevelSource::hasChunk(int x, int y)
{
	return true;
}

void FarlandsLevelSource::calcWaterDepths(ChunkSource *parent, int xt, int zt)
{
	int xo = xt * 16;
	int zo = zt * 16;
	for (int x = 0; x < 16; x++)
...
void FarlandsLevelSource::postProcess(ChunkSource *parent, int xt, int zt)
{
...
bool FarlandsLevelSource::save(bool force, ProgressListener *progressListener)
{
	return true;
}

bool FarlandsLevelSource::tick()
{
	return false;
}

bool FarlandsLevelSource::shouldSave()
{
	return true;
}

wstring FarlandsLevelSource::gatherStats()
{
	return L"FarlandsLevelSource";
}

vector<Biome::MobSpawnerData *> *FarlandsLevelSource::getMobsAt(MobCategory *mobCategory, int x, int y, int z)
{
...
TilePos *FarlandsLevelSource::findNearestMapFeature(Level *level, const wstring& featureName, int x, int y, int z)
{
...
void FarlandsLevelSource::recreateLogicStructuresForChunk(int chunkX, int chunkZ)
{
	if (generateStructures)
	{
