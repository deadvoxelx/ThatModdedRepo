...
bool GlowingNustoneFeature::place(Level *level, Random *random, int x, int y, int z)
{
	if (!level->isEmptyTile(x, y, z)) return false;
	if (random->nextInt(3) == 0)
	{
		level->setTileAndData(x, y, z, Tile::glowingNustone_Id, 0, Tile::UPDATE_CLIENTS);
		for (int i = 0; i < 1500; i++)
		{
			...
			for (int t = 0; t < 6; t++)
			{
				...
				if (tile == Tile::glowingNustone_Id) count++;
			}

			if (count == 1) level->setTileAndData(x2, y2, z2, Tile::glowingNustone_Id, 0, Tile::UPDATE_CLIENTS);
		}
		return true;
	}
}