...
const double DartNethanium::DART_BASE_DAMAGE = 3.0f;

void DartNethanium::_init()
{
...
DartNethanium::DartNethanium(Level *level) : Entity( level )
{
	_init();
...
DartNethanium::DartNethanium(Level *level, shared_ptr<LivingEntity> mob, shared_ptr<LivingEntity> target, float power, float uncertainty) : Entity( level )
{
	_init();
...
DartNethanium::DartNethanium(Level *level, double x, double y, double z) : Entity( level )
{
	_init();
...
DartNethanium::DartNethanium(Level *level, shared_ptr<LivingEntity> mob, float power) : Entity( level )
{
	_init();
...
void DartNethanium::defineSynchedData()
{
	entityData->define(ID_FLAGS, static_cast<byte>(0));
}

void DartNethanium::shoot(double xd, double yd, double zd, float pow, float uncertainty)
{
...
void DartNethanium::lerpTo(double x, double y, double z, float yRot, float xRot, int steps)
{
	setPos(x, y, z);
	setRot(yRot, xRot);
}

void DartNethanium::lerpMotion(double xd, double yd, double zd)
{
	this->xd = xd;
	this->yd = yd;
	this->zd = zd;
...
void DartNethanium::tick() 
{
	...
	if (res != nullptr)
	{
		if (res->entity != nullptr)
		{
			...
			if (owner == nullptr)
			{
				damageSource = DamageSource::dartNethanium(dynamic_pointer_cast<DartNethanium>(shared_from_this()), shared_from_this());
			}
			else
			{
				damageSource = DamageSource::dartNethanium(dynamic_pointer_cast<DartNethanium>(shared_from_this()), owner);
			}
...
void DartNethanium::addAdditonalSaveData(CompoundTag *tag)
{
...
void DartNethanium::readAdditionalSaveData(CompoundTag *tag)
{
...
void DartNethanium::playerTouch(shared_ptr<Player> player)
{
	...
	{
		if (!player->inventory->add(std::make_shared<ItemInstance>(Item::dartNethanium, 1)))
		{
			bRemove = false;
		}
	}
...
bool DartNethanium::makeStepSound()
{
	return false;
}

float DartNethanium::getShadowHeightOffs()
{
	return 0;
}

void DartNethanium::setBaseDamage(double baseDamage)
{
	this->baseDamage = baseDamage;
}

double DartNethanium::getBaseDamage()
{
	return baseDamage;
}

void DartNethanium::setKnockback(int knockback)
{
	this->knockback = knockback;
}

bool DartNethanium::isAttackable()
{
	return false;
}

void DartNethanium::setCritDart(bool critDart)
{
