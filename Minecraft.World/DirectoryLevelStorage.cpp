...
void DirectoryLevelStorage::resetAetherPlayerPositions()
{
	if(app.GetResetAether())
	{
#if defined(__PS3__) || defined(__ORBIS__) || defined(__PSVITA__)
		vector<FileEntry *> *playerFiles = m_saveFile->getValidPlayerDatFiles();
#else
...
void DirectoryLevelStorage::resetEndPlayerPositions()
{
	if(app.GetResetEnd())
	{
#if defined(__PS3__) || defined(__ORBIS__) || defined(__PSVITA__)
		...
		if ( playerFiles )
		{
			for ( FileEntry * realFile : *playerFiles )
			{
				...
				if (tag != nullptr)
				{
					if(tag->contains(L"Dimension") && tag->getInt(L"Dimension") == LevelData::DIMENSION_END && tag->contains(L"Pos"))
					{
						ListTag<DoubleTag> *pos = (ListTag<DoubleTag> *) tag->getList(L"Pos");
						pos->get(1)->data = DBL_MAX;
...
void DirectoryLevelStorage::resetOuterEndPlayerPositions()
{
	if(app.GetResetOuterEnd())
	{
#if defined(__PS3__) || defined(__ORBIS__) || defined(__PSVITA__)
		vector<FileEntry *> *playerFiles = m_saveFile->getValidPlayerDatFiles();
#else
