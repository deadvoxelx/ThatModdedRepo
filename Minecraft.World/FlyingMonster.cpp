...
FlyingMonster::FlyingMonster(Level *level) : PathfinderMob( level )
{
	xpReward = Enemy::XP_REWARD_MEDIUM;
}

void FlyingMonster::aiStep() 
{
	updateSwingTime();
...
shared_ptr<Entity> FlyingMonster::findAttackTarget()
{
	...
	shared_ptr<Player> player = level->getNearestAttackablePlayer(shared_from_this(), 24);
	if (player != nullptr && canSee(player) ) return player;
	return shared_ptr<Player>();
}

bool FlyingMonster::hurt(DamageSource *source, float dmg)
{
...
* @return
*/
bool FlyingMonster::doHurtTarget(shared_ptr<Entity> target)
{
...
void FlyingMonster::checkHurtTarget(shared_ptr<Entity> target, float distance)
{
...
float FlyingMonster::getWalkTargetValue(int x, int y, int z)
{
	return 0.5f;
}

bool FlyingMonster::isDarkEnoughToSpawn()
{
	int xt = Mth::floor(x);
...
void FlyingMonster::causeFallDamage(float distance)
{
}

void FlyingMonster::checkFallDamage(double ya, bool onGround)
{
}

void FlyingMonster::travel(float xa, float ya)
{
	if (isInWater())
	{
