...
NurealmLevelSource::NurealmLevelSource(Level *level, int64_t seed)
{
	m_XZSize = NUREALM_LEVEL_MIN_WIDTH;
...
void NurealmLevelSource::prepareHeights(int xOffs, int zOffs, byteArray blocks, BiomeArray biomes)
{
...
void NurealmLevelSource::buildSurfaces(int xOffs, int zOffs, byteArray blocks, BiomeArray biomes)
{
	...
	for (int x = 0; x < 16; x++)
	{
		for (int z = 0; z < 16; z++)
		{
			...
			byte top = (byte) Tile::nugrass_Id;
			byte material = (byte) Tile::nustone_Id;
			byte base = (byte) Tile::nustone_Id;
				...
				int offs = (z * 16 + x) * Level::genDepth + y;
				{
					...
					else if (old == Tile::nustone_Id)
					{
						if (run == -1)
						{
							...
							{
								top = (byte) Tile::nugrass_Id;
								material = (byte) Tile::nustone_Id;
								if (gravel) top = (byte) Tile::nugrass_Id;
								if (gravel) material = (byte) Tile::nustone_Id;
								if (sand) top = (byte) Tile::nugrass_Id;
								if (sand) material = (byte) Tile::nustone_Id;
							}*/
							run = runDepth;
							blocks[offs] = top;
						}
...
LevelChunk *NurealmLevelSource::create(int x, int z)
{
	return getChunk(x, z);
}

LevelChunk *NurealmLevelSource::getChunk(int xOffs, int zOffs)
{
...
doubleArray NurealmLevelSource::getHeights(doubleArray buffer, int x, int y, int z, int xSize, int ySize, int zSize)
{
	...
	for (int xx = 0; xx < xSize; xx++)
	{
		...
		{
			double scale = ((sr[pp] + 128.0) / 256);
			if (scale > 1) scale = 1;

			double depth = (dr[pp] / 4000.0);
...
bool NurealmLevelSource::hasChunk(int x, int y)
{
	return true;
}

void NurealmLevelSource::calcWaterDepths(ChunkSource *parent, int xt, int zt)
{
	int xo = xt * 16;
	int zo = zt * 16;
	for (int x = 0; x < 16; x++)
...
void NurealmLevelSource::postProcess(ChunkSource *parent, int xt, int zt)
{
...
bool NurealmLevelSource::save(bool force, ProgressListener *progressListener)
{
	return true;
}

bool NurealmLevelSource::tick()
{
	return false;
}

bool NurealmLevelSource::shouldSave()
{
	return true;
}
...
TilePos *NurealmLevelSource::findNearestMapFeature(Level *level, const wstring& featureName, int x, int y, int z)
{
	return nullptr;
}

void NurealmLevelSource::recreateLogicStructuresForChunk(int chunkX, int chunkZ)
{
}
